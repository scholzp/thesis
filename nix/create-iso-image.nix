...
# Actual args.
{
  kernel,
  initrd,
  kernelCmdline ? "nr_cpus=3 memmap=${low_mem_size}!${start_address} nox2apic loglevel=7 nokaslr",
}@args:
...
let
    ...
    (writeTextFile {
      ...
      text = ''
        ...
        /Boot ${baseNameOf kernel}
            ...
            protocol:linux
            kernel_path: boot():/${baseNameOf kernel}/bzImage
            kernel_cmdline: nr_cpus=3 console=ttyS0 memmap=${low_mem_size}!${start_address} nox2apic loglevel=7 nokaslr
  ...
  createHybridMultibootIso =
    ...
    let
      ...
      copyBootitemsLines = map (elem: "cp -r ${elem} -f filesystem/${builtins.baseNameOf elem}") bootItems;
    in
    runCommand "${kernel.name}-multiboot2-hybrid-iso"
      ...
      ''
        ...
        cp ${initrd}/initrd -f filesystem/initrd
        ${builtins.concatStringsSep "\n" copyBootitemsLines}
