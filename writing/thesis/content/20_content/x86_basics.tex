...
same \acrshort{a_isa} on the microarchitectural plane. I, therefore, note
...
\acrshort{a_gdt} allowed virtual memory implementation with memory protection. A
new \gls{mmu}, short \acrshort{a_mmu}, added for the first time on chip in the
...
\acrshort{a_idt} is provided by writing to the IDTR before interrupts can be
activated. \\
...
the \acrfull{a_llc} is shared among all cores for synchronization and uses one
of the coherency protocols. CPUs produced by Intel largely use an inclusive
\acrshort{a_llc}. An inclusive cache describes a cache containing items in lower
...
microarchitecture.\cite{intel_sdm} The x86\_64 \acrshort{a_isa} specificities
four freely programmable architectural hardware
\acrshort{a_pmc}s.\cite{amd_manual} Concrete processor implementation can offer
additional counters. Similarly, the \acrshort{a_isa} specifies architectural
...
supports in the result of the \gls{CPUID} instruction. \\
...
When I use hardware \acrshort{a_pmc}, I must use the proper technique adopted to
